...
9. 不得在末尾凭空补充原文中没有的内容：**禁止**在译文末尾追加原文里不存在的列表项、段落、总结句或重复内容。如果分段在原文中以「3 条 bullets + 1 段收束」结尾，译文必须以「3 条 bullets + 1 段收束」结尾，不得变成「3 条 + 1 段 + 重复 3 条」「3 条 + 1 段 + 多 1 条 bullets」或「3 条 + 1 段 + 重复总结句」。把原文有的内容**完整翻译完一次**就停，不要为了「凑齐」「润色」「补全」而再次复述前文。
`.trim();

...
3. 人名、机构名、专有术语、关键概念第一次出现时必须中英文对照，优先使用“中文（英文）”格式；文章标题、各级标题、列表项中的首次出现也算首次出现。若某一列表或并列结构中首次出现多个州名、地区名、风暴名或术语，要逐项补齐，不要只给列表整体补一次。注意：中英文对照只针对这些局部元素本身，不要把整条标题、导语句、小标题或列表项原句整句附上英文。
4. 缩写第一次出现时，优先写成“中文全称（英文全称，缩写）”或最自然的中英文对照形式；后文保持译法一致。
...
输出前请自行检查：
- 段落数是否与原文完全一致。
- 首次出现的人名、机构名、关键术语是否都已中英文对照，包括标题、各级标题、列表项和正文中的第一次出现。
...
2. first_mention_bilingual：人名、机构名、关键术语第一次出现时是否完成中英文对照；标题、各级标题、列表项和正文中的第一次出现都要检查。遇到州名、地区名、风暴名、并列术语或列表枚举时，要逐项检查。同时要判定对照范围是否过宽；如果把整条标题、导语句、小标题或列表项原句整句附上英文，也应判为不通过。
...
6. 对“首次出现中英文对照”问题，修复后的首次出现必须直接写成“中文（英文）”或等价的自然双语形式；标题、各级标题、列表项也适用，但只补局部专名或术语，不得把整条标题、导语句、小标题或列表项原句整句附上英文。
...
10. 不要引入新的中英对照遗漏、数字单位错误、标点错误或单位换算边界错误。
